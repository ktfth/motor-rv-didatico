...
|---|---|---|---|
| I1 | Para cada conta × instrumento: `disponivel + Σ a_liquidar_venda[D] + bloqueado + sobras` == posição na depositária no fechamento (**enunciado corrigido — ver "Correções"**) | `ReconciliacaoDepositaria` (EOD) e assert em debug | — |
...
| I13 | Para cada conta × instrumento: `disponivel + Σ a_liquidar_compra[D] + bloqueado + sobras` == posição projetada depois de liquidar tudo que está pendente | assert em debug após cada apply | — |
